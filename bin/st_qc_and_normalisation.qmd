...
```
We show the data on the spatial axis 
```{python}
sc.pl.spatial(st_adata, color = ["total_counts", "n_genes_by_counts",'pct_counts_mt'])
```

We show the most expressed genes in the dataset
```{python}
